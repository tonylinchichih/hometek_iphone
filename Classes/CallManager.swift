...
@objc class CallManager: NSObject {
	...
	func displayIncomingCall(call:Call?, handle: String, hasVideo: Bool, callId: String) {
		...
		providerDelegate.reportIncomingCall(call:call, uuid: uuid, handle: convertAddress(address: handle), hasVideo: hasVideo)
	}
    
    var someDict:[String:String] = ["0011":"管理機1",
                                    "0012":"管理機2",
                                    "0013":"管理機3",
                                    "0014":"管理機4",
                                    "0015":"管理機5",
    ...
    @objc func convertAddress(address: String) -> String {
        if address.count == 0 {
            return "Hometek";
        }
        
        if address.count == 4 {
            if let val = someDict[address] {
                return val
            }
        } else if address.count == 8 {
            // 0158010101:  158號1樓1室
            let number = Int(address[0..<4]) ?? 0
            let floor = Int(address[4..<6]) ?? 0
            let room = Int(address[6...]) ?? 0
            let aStr = String(format: "%d號%d樓%d室", number, floor, room)
            return aStr
        }
...
class CoreManagerDelegate: CoreDelegate {
	...
	override func onCallStateChanged(lc: Core, call: Call, cstate: Call.State, message: String) {
		...
		var address = FastAddressBook.displayName(for: addr?.getCobject) ?? "Unknow"
        address = CallManager.instance().convertAddress(address: address)
